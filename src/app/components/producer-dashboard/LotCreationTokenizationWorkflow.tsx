...
const LotCreationTokenizationWorkflow: React.FC = () => {
    ...
    <div className="space-y-6">
      ...
      {selectedBatch && (
        <Card className="p-4 sm:p-6 bg-white shadow-sm border border-gray-200">
          ...
          <Button onClick={handleAddLot} className="mt-4 bg-[#FFD700] text-[#003E6D] hover:bg-[#FFD700]/90 min-h-[44px]">
                ...
                <div className="flex justify-between items-start gap-2">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <Package className="w-4 h-4 text-[#003E6D] flex-shrink-0" />
                      <span className="truncate">{lot.lotNumber}</span>
                      {getGradeBadge(lot.grade)}
                      {getStatusBadge(lot.status)}
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      <p className="text-gray-600">Qty: {lot.quantity} {lot.unit}</p>
                      <p className="text-gray-600">{new Date(lot.createdAt).toLocaleDateString()}</p>
                      {lot.tokenId && (
                        <p className="text-gray-600 col-span-2 break-all text-xs">Token: {lot.tokenId}</p>
                      )}
                    </div>
                  </div>
      ...
      {lots.length > 0 && (
        <div className="flex justify-end">
          <Button
            ...
            className="bg-[#003E6D] text-white hover:bg-[#003E6D]/90 min-h-[44px]"
          >
    ...
    <div className="space-y-6">
      <Card className="p-4 sm:p-6 bg-white shadow-sm border border-gray-200">
        <div className="flex flex-wrap justify-between items-start gap-3 mb-6">
          ...
          <Button
            ...
            className="bg-[#FFD700] text-[#003E6D] hover:bg-[#FFD700]/90 min-h-[44px]"
        ...
        <div className="grid grid-cols-3 gap-2 sm:gap-4 mb-6">
          <div className="p-3 sm:p-4 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg">
            <p className="text-gray-600 text-xs sm:text-sm">Total Lots</p>
            <p className="text-xl sm:text-2xl">{lots.length}</p>
          </div>
          <div className="p-3 sm:p-4 bg-gradient-to-br from-yellow-50 to-yellow-100 rounded-lg">
            <p className="text-gray-600 text-xs sm:text-sm">Pending</p>
            <p className="text-xl sm:text-2xl">{lots.filter(l => l.status === 'pending').length}</p>
          </div>
          <div className="p-3 sm:p-4 bg-gradient-to-br from-green-50 to-green-100 rounded-lg">
            <p className="text-gray-600 text-xs sm:text-sm">Tokenized</p>
            <p className="text-xl sm:text-2xl">{lots.filter(l => l.status === 'tokenized').length}</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          {lots.map((lot) => (
            <div
              key={lot.id}
              className="p-4 sm:p-5 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg border border-blue-200"
            >
              <div className="flex flex-wrap justify-between items-start gap-3 mb-3">
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2 mb-2">
                    <QrCode className="w-5 h-5 text-[#003E6D] flex-shrink-0" />
                    <span className="truncate">{lot.lotNumber}</span>
                    {getGradeBadge(lot.grade)}
                    {getStatusBadge(lot.status)}
                  </div>
                ...
                <Button
                  ...
                  className="bg-[#003E6D] text-white hover:bg-[#003E6D]/90 disabled:opacity-50 min-h-[44px] flex-shrink-0"
                >
                ...
                <div className="mt-3 p-3 bg-white rounded border border-green-300">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
                    <div>
                      ...
                      <p className="font-mono text-xs break-all">{lot.globalBatchId}</p>
                    </div>
                    <div>
                      <p className="text-gray-600">Token ID</p>
                      <p className="font-mono text-xs break-all">{lot.tokenId}</p>
                    </div>
                  </div>
                </div>
              )}
      ...
      <div className="flex flex-wrap justify-between gap-3">
        <Button
          ...
          variant="outline"
          className="border-[#003E6D] text-[#003E6D] min-h-[44px]"
        >
          ...
          <Tabs defaultValue={tokenizedLots[0]?.id || ''} className="w-full">
            <TabsList className="grid w-full" style={{ gridTemplateColumns: `repeat(${tokenizedLots.length}, 1fr)` }}>
              {tokenizedLots.map((lot) => (
                <TabsTrigger key={lot.id} value={lot.id} className="text-xs sm:text-sm truncate">
                  {lot.lotNumber}
                </TabsTrigger>
              ))}
            </TabsList>
                    ...
                    <div className="flex flex-wrap justify-between items-center gap-2 mb-3">
                      ...
                      <Dialog>
                        <DialogTrigger asChild>
                          <Button size="sm" variant="outline" className="border-[#003E6D] text-[#003E6D] min-h-[44px]">
                        ...
                        <DialogContent>
                          ...
                          <div className="space-y-4">
                            ...
                            <Button
                              ...
                              className="w-full bg-[#003E6D] text-white min-h-[44px]"
                            >
                          ...
                          <div key={cert.id} className="p-3 bg-gray-50 rounded-lg border border-gray-200">
                            <div className="flex justify-between items-start gap-2">
                              <div className="min-w-0">
                                <p className="truncate">{cert.name}</p>
                                <p className="text-sm text-gray-600 break-words">
                                  {cert.type} • Uploaded by {cert.uploadedBy} • {new Date(cert.uploadedAt).toLocaleDateString()}
                                </p>
                              </div>
                              <Button size="sm" variant="ghost" className="flex-shrink-0 min-h-[44px]">
                          ...
                          <div key={verification.id} className="p-3 bg-green-50 rounded-lg border border-green-200">
                            <div className="flex flex-wrap justify-between items-start gap-2 mb-1">
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                <span className="truncate">{verification.verifierName}</span>
                              </div>
                      ...
                      {tokenData?.cultivationHistory.map((entry, index) => (
                        <div key={entry.id} className="flex gap-3">
                          <div className="flex flex-col items-center">
                            <div className="w-8 h-8 rounded-full bg-[#003E6D] text-white flex items-center justify-center text-sm flex-shrink-0">
                              {index + 1}
                            </div>
  ...
  const renderBuyerView = () => {
    ...
    return (
      <div className="space-y-6">
        ...
        <Card className="p-4 sm:p-6 bg-gradient-to-br from-[#F7FAFC] to-[#D9F2FF] shadow-lg border-2 border-[#003E6D]">
        ...
        {buyerViewSelectedLot && tokenData && (
          ...
          >
            ...
            <Card className="p-4 sm:p-6 bg-white shadow-sm border border-gray-200">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="font-mono text-xs mt-1 break-all">{tokenData.tokenId}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-lg">
                  <p className="text-gray-600 text-xs">Lot Number</p>
                  <p className="mt-1 text-sm">{buyerViewSelectedLot.lotNumber}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-lg">
                ...
                <div className="p-3 bg-gray-50 rounded-lg">
                  ...
                  <p className="mt-1 text-sm">{buyerViewSelectedLot.quantity} {buyerViewSelectedLot.unit}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-lg">
                  <p className="text-gray-600 text-xs">Batch ID</p>
                  <p className="mt-1 text-sm">{buyerViewSelectedLot.batchId}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-lg">
                  <p className="text-gray-600 text-xs">Global Batch ID</p>
                  <p className="font-mono text-xs mt-1 break-all">{tokenData.globalBatchId}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-lg">
                  <p className="text-gray-600 text-xs">Commodity</p>
                  <p className="mt-1 text-sm">{selectedBatch?.commodity || 'N/A'}</p>
                </div>
                <div className="p-3 bg-gray-50 rounded-lg">
                  <p className="text-gray-600 text-xs">Harvest Date</p>
                  <p className="mt-1 text-sm">{selectedBatch ? new Date(selectedBatch.harvestDate).toLocaleDateString() : 'N/A'}</p>
                </div>
              </div>
            </Card>
            ...
            {tokenData.aiInsights && (
              <Card className="p-4 sm:p-6 bg-gradient-to-br from-purple-50 to-purple-100 shadow-sm border border-purple-200">
                <h3 className="mb-4 flex items-center gap-2">
              ...
              <div className="space-y-4">
                {tokenData.cultivationHistory.map((entry, index) => (
                  <div key={entry.id} className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#003E6D] to-[#0066CC] text-white flex items-center justify-center flex-shrink-0">
                        {index + 1}
                      </div>
                      {index < tokenData.cultivationHistory.length - 1 && (
                    ...
                    <div className="flex-1 min-w-0 pb-8">
                      <div className="p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg">
                        <div className="flex flex-wrap justify-between items-start gap-2 mb-2">
                          ...
                          {entry.verifiedBy && (
                            <Badge variant="outline" className="border-green-500 text-green-700 flex-shrink-0">
                    ...
                    {tokenData.certificates.map((cert) => (
                      <div key={cert.id} className="p-3 bg-green-50 rounded-lg border border-green-200">
                        <div className="flex flex-wrap justify-between items-start gap-2 mb-1">
                          ...
                          <Badge className="bg-green-500 text-white text-xs flex-shrink-0">{cert.type}</Badge>
                        </div>
                    ...
                    <div key={verification.id} className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex flex-wrap justify-between items-start gap-2 mb-1">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <span className="text-sm truncate">{verification.verifierName}</span>
                        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F7FAFC] to-[#D9F2FF] p-4 sm:p-6 overflow-x-hidden">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-6 sm:mb-8">
          <h1 className="text-[#003E6D] mb-2 text-xl sm:text-2xl">Lot Creation & Tokenization Workflow</h1>
          <p className="text-gray-600 text-sm sm:text-base">Create lots, tokenize on blockchain, enrich data, and preview buyer view</p>
        </div>

        {/* Progress Steps */}
        <Card className="p-4 sm:p-6 mb-6 sm:mb-8 bg-white shadow-sm border border-gray-200">
          <div className="flex justify-between items-center relative">
            ...
            {steps.map((step) => {
              ...
              return (
                ...
                >
                  <motion.div
                    className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center mb-1 sm:mb-2 cursor-pointer ${
                      isActive
                  ...
                  <p className={`text-xs text-center leading-tight ${isActive ? 'text-[#003E6D] font-medium' : 'text-gray-600'}`}>
                    {step.label}
                  </p>
                </div>
              );
