  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#F7FAFC] to-[#D9F2FF] p-4 sm:p-6 overflow-x-hidden">
      <div className="max-w-6xl mx-auto">
          ...
          {filteredMatches.map((match) => (
            <DSCard key={match.id} className="hover:shadow-lg transition-shadow">
              <div className="p-4 sm:p-6">
                ...
                <div className="flex items-start justify-between gap-3 mb-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-2">
                      <h3 className="text-[#003E6D] break-words">{match.name}</h3>
                      {match.verified && (
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-sm text-gray-600">
                      ...
                      <span className="flex items-center gap-1">
                        <Star className="w-4 h-4 fill-[#FFD700] text-[#FFD700] flex-shrink-0" />
                  ...
                  <div className="text-center flex-shrink-0">
                    <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-gradient-to-br from-[#FFD700] to-[#FFD700]/50 flex items-center justify-center mb-1">
                      <span className="text-[#003E6D] text-sm font-semibold">{match.matchScore}%</span>
                    </div>
                ...
                {/* Details Grid */}
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4 p-3 sm:p-4 bg-gradient-to-br from-[#F7FAFC] to-[#D9F2FF]/30 rounded-lg">
                  <div>
                  ...
                  <div>
                    ...
                    <span className="text-[#003E6D] text-xs sm:text-sm break-words">{match.priceRange}</span>
                  </div>
                </div>

                {/* Action Buttons */}
                <div className="flex flex-col sm:flex-row gap-2">
                  <DSButton variant="primary" className="w-full sm:flex-1 flex items-center justify-center gap-2 min-h-[44px]">
                    <TrendingUp className="w-4 h-4" />
                    Propose Price
                  </DSButton>
                  <DSButton variant="outline" className="w-full sm:flex-1 flex items-center justify-center gap-2 min-h-[44px]">
        ...
        {/* Back Button */}
        {onBack && (
          <DSButton variant="outline" onClick={onBack} className="w-full min-h-[44px]">
            Back to Dashboard
          </DSButton>
        )}
      </div>
